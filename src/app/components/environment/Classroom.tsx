...
export const Classroom = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const concentration = response.data.beta + response.data.alpha - (response.data.theta - 0.1);
	...
	return (
		<>
			...
			<audio hidden ref={audioRef} controls onLoad={audioRef.current?.play} loop={true}>
				<source src={rainSound} type="audio/mp3" />
			</audio>
			<Canvas>
				<directionalLight ref={directionalLightRef} position={[0, 20, -30]} intensity={Focus + 0.1} color={"white"} castShadow />
				...
				<PerspectiveCamera makeDefault position={[-7, 4, 2]} fov={65} near={0.1} far={100} />
